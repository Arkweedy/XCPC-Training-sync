...
void solve()
{
    ...
    auto calcl = [&](int l, int r)->int
    {
        ...
        while(p != 0){
            ...
            int c = min(complen, t[p].len - t[fp].len) / d;
            int len1 = t[p].len - d * c, len2 = t[p].len - d * (c + 1);
            if(len1 >= 0)ans = max(ans, len1 + 2 * get(l - len1, r));
            if(len2 >= 0 && len2 + 1 <= l)ans = max(ans, len2 + 2 * get(l - len2, r));
            p = fp;
        }
        return ans;
    };
    ...
    auto calcr = [&](int l, int r)->int
    {
        ...
        while(p != 1){
            int fp = rt[p].slink;
            int d = rt[p].diff;
            int complen = get(l, r + rt[fp].len);
            int c = min(complen, rt[p].len - rt[fp].len) / d;
            int len1 = rt[p].len - d * c, len2 = rt[p].len - d * (c + 1);
            if(len1 >= 0)ans = max(ans, len1 + 2 * get(l, r + len1));
            if(len2 >= 0 && r + len2 <= n)ans = max(ans, len2 + 2 * get(l, r + len2));
            p = fp;
        }
        return ans;
    };
