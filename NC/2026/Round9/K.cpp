...
//Create time: 2026.08.14 17:42:17
...
void solve()
{
    ...
    auto build = [&](auto&&self, int l, int r, int p)->void
    {
        ...
        int m = l + r >> 1;
        self(self, l, m, p * 2);
        self(self, m + 1, r, p * 2 + 1);
        H[p] = H[p * 2] * H[p * 2 + 1];
        return;
    };
    build(build, 0, n - 1, 1);

    // F_r = F_{l - 1} * H_{l, r} + D_{l, r}
    // F_n = 

    vector<poly>D(n << 2);
    auto cdq = [&](auto&&self, int l, int r, int p, poly In)->void
    {
        if(l == r){
            int k = a[l];
            D[p] = poly(k * 2);
        ...
        int m = l + r >> 1;
        int Llen = Len(l, m), Rlen = Len(m + 1, r), len = Len(l, r);

        poly Lin(In.begin(),In.begin() + Llen);
        self(self, l, m, p * 2, std::move(Lin));

        poly Rin = (In * H[p * 2] + D[p * 2]).shift(-Llen).trunc(Rlen);
        self(self, m + 1, r, p * 2 + 1, std::move(Rin));
        

        D[p] = D[p * 2] * H[p * 2 + 1] + D[p * 2 + 1].shift(Llen);
        return;
    };
    poly in(S, 0);
    in[0] = 1;
    cdq(cdq, 0, n - 1, 1, std::move(in));
    
    auto E = (D[1] + H[1]).shift(-S);
    i64 ans = 0;
