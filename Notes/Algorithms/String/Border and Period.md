...
> 若 $a,b \in P(s)$ 且 $a + b - \gcd(a, b) \leq |s|$, 那么有： $$\gcd(a,b) \in P(s)$$

先引入一些周期的性质：
1. 如果字符串的最小周期可以整除字符串的长度，那么字符串的所有周期都是字符串最小周期的整数倍，反之则不一定。证明是显然的。逆命题的一个典型例子的斐波那契串，如abaab。
2. 弱周期定理：
    > 若 $a,b \in P(s)$ 且 $a + b \leq |s|$, 那么有： $$\gcd(a,b) \in P(s)$$
