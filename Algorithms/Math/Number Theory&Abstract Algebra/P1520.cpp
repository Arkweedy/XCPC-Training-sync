...
void output(poly& f)
{
    ...
    for(int i = f.size() - 1;i >= 0;i--){
        if(f[i] != 0){
            ll x = f[i].x > P / 2 ? f[i].x - P : f[i].x;
            if(x < 0)cout<<"-";
            else if(i != f.size() - 1) cout<<"+";
            if((x != 1 && x != -1) || i == 0)cout<<abs(x);
            if(i != 0)cout<<"x";
            if(i > 1)cout<<"^"<<i;
        }
    }
...
void solve()
{
    ...
    for(int i = 2;i <= n;i++){
        if(n % i != 0)continue;//only get phi(d), d|n
        if(isprime[i]){
            prime.push_back(i);
            cycpoly[i].resize(i, 1);
            phi[i] = i - 1;
            lpfp[i] = i;
        }
        for(auto p : prime){
            if(i * p > n)break;
            isprime[i * p].flip();
            poly npoly(cycpoly[i].size() * p - p + 1);
            for(int j = 0;j < cycpoly[i].size();j++){
                npoly[j * p] = cycpoly[i][j];
            }
            cycpoly[i * p] = move(npoly);//phi_n(x^p)
            if(i % p != 0){
                lpfp[i * p] = p;
                phi[i * p] = phi[i] * phi[p];
                //phi_n(x^p) / phi_n(x)
                cycpoly[i * p] *= cycpoly[i].inv(cycpoly[i * p].size());
                cycpoly[i * p].resize(phi[i * p] + 1);
            }
            else{
                lpfp[i * p] = lpfp[i] * p;
                if(lpfp[i * p] == i * p)phi[i * p] = i * phi[p];
                else phi[i * p] = phi[i / lpfp[i]] * phi[lpfp[i * p]];
                break;
            }
        }
    }
    ...
    for(int i = 1;i <= n;i++){
        if(n % i == 0){
            ans.push_back(cycpoly[i]);
        }
    }

    sort(ans.begin(),ans.end(),[&](poly& a, poly& b)->bool
    {
        if(a.size() != b.size())return a.size() < b.size();
        for(int i = a.size() - 1;i >= 0;i--){
            if(a[i] != b[i]){
                int x = a[i].x > P / 2 ? a[i].x - P : a[i].x;
                int y = b[i].x > P / 2 ? b[i].x - P : b[i].x;
    ...
    // for(int i = 1;i <= n;i++){
    //     cerr<<phi[i]<<" "<<cycpoly[i].size()<<endl;
    //     output(cycpoly[i]);
    // }
    return;
}
