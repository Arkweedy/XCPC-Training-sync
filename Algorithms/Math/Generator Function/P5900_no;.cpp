...
void solve() {
    ...
    poly F(n + 1), G(n + 1), need(n + 2);  // 1-based；need[i] 存放右端和式，F[i] = need[i]/(i-1)
    F[1] = 1;

    // 把 F[1] 的倍数筛进 G
    for (int k = 1; k <= n; k += 1) G[k] += Z(1) * F[1];

    // 立刻把 (F[1] * G[1]) 贡献到 need[2]，保证 F[2] 可算
    if (n >= 2) need[2] += F[1] * G[1];

    // 块：记录连续后缀 [L .. L+len-1]，len 必为 2^k
    struct Block { int L, len; };
    std::vector<Block> stk;
    stk.reserve(64);
    stk.push_back({1, 1});

    auto convolve_right_with_G_segment = [&](int rightL, int len) {
        // 仅卷：F[rightL .. rightL+len-1] × G[len+1 .. 2*len]
        poly f(len), g(len);
        for (int i = 0; i < len; ++i) f[i] = F[rightL + i];
        for (int i = 0; i < len; ++i) g[i] = G[len + 1 + i];
        poly h = f * g; // h[w] 对应 s = (rightL + (len + 1)) + w

        int s0 = rightL + (len + 1);
        int s1 = std::min(n, rightL + (3 * len - 1)); // 最大 s
        for (int s = s0; s <= s1; ++s) {
            need[s] += h[s - s0];
        }
    };
    ...
    for (int t = 2; t <= n; ++t) {
        ...
        // 把 F[t] 筛进 G 的倍数位（全局一次性，保持 G 前缀始终正确）
        for (int k = t; k <= n; k += t) {
            G[k] += Z(t) * F[t];
        }

        // 立刻把 (F[t] * G[1]) 计入 need[t+1]
        if (t + 1 <= n) need[t + 1] += F[t] * G[1];

        // 压入长度为 1 的新块
        stk.push_back({t, 1});

        // 只要栈顶两个块长度相等，就合并，并在合并瞬间做“新右半 × G区间”的卷积
        while (stk.size() >= 2) {
            auto b2 = stk.back();         // 右块
            auto b1 = stk[stk.size() - 2];// 左块
            if (b1.len != b2.len) break;

            // 合并成 [b1.L .. b1.L + 2*len - 1]，右半起点：
            int len = b2.len;
            int rightL = b1.L + len;

            // 只在这一层把“右半 × G[len+1 .. 2*len]”计入 need[*]
            convolve_right_with_G_segment(rightL, len);

            // 栈上替换为合并后的大块
            stk.pop_back();                    // 去掉右块
            stk.back().len = len * 2;          // 左块扩成更大块
        }
    }

    // 之后按你的题意进行后处理（下为你原逻辑示例）：
    for(int i = 1;i <= n;i++){
        cerr<<F[i]<<" ";
    }
    cerr<<endl;
    ...
    if (n % 2 == 0) ans -= F[n / 2] * (F[n / 2] - Z(1)) / Z(2);
    std::cout << ans << '\n';
}
