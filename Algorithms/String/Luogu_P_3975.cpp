...
struct SAM
{
    ...
    {
        string_all_cnt.resize(endpos_size.size());
        for(int i = 0;i < t.size();i++){
            string_all_cnt[i] = endpos_size[i];
        }
...
void solve()
{
    ...
    auto & t = sam.t;
    if(op == 0){
        sam.calc_dif_string_cnt();
        auto &dcnt = sam.string_dif_cnt;
        string ans;
        auto dfs = [&](auto&&self, int p)->void
        {
            k--;
        ...
        sam.calc_endpos_size(s);
        sam.calc_all_string_cnt();
        auto &acnt = sam.string_all_cnt;
        auto &ecnt = sam.endpos_size; 
        string ans;
        auto dfs = [&](auto&&self, int p)->void
        {
            k -= ecnt[p];
