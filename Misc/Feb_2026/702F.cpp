...
//702F.cpp Create time : 2026.02.10 16:55
...
struct Treap
{
    struct Node{
        ...
        int res;
        int ktag, rtag;
        Node():ls(0),rs(0),pri(rng()),key(),sz(0),cnt(0), res(0),ktag(0), rtag(0){}
    };
...
void solve()
{
    ...
    for(auto [c, p] : a){
        auto [x, y] = t.split_less(t.rt, {c, -1});
        auto [z, w] = t.split_less(y,{c * 2, -1});
        t.apply(z, -c, 1);
        x = t.unite(x, z);
        t.apply(w, -c, 1);
        t.rt = t.merge(x, w);
