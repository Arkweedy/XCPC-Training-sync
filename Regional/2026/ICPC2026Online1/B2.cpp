...
//B2.cpp Create time : 2026.09.09 15:20
...
void solve()
{
    ...
    //  for x in [1, v] dec
    //      f[x mod a_i] += f[x] / pc[x]
    //  y = x mod a_i -> x = y + k a_i , y < a_i 
    //  f[y] += \sum_{d | a} f[y + d] / pc[y + d] , a > y
    //  f[y] += \sum_{d >= 1} h[d] f[y + d] / pc[y + d] , a > y

    auto cdq = [&](this auto&&cdq, int l, int r)->void
    {
        if(l == r){
            if(pc[r] > 0)f[r] = 1ll * f[r] * inv(pc[r]) % P;
            return;
        }
        int m = l + r >> 1;
        int len = r - l + 1;
        ...
        if(len > sq){
            ...
            for(int i = m + 1;i <= r;i++){
                if(c[i] > 0){
                    for(int j = 1;j <= (v - l) / i;j++){
                        h[i * j - b] += c[i];
                    }
                }
            }
            auto g = poly(f.begin() + b + l, f.end());
            auto w = g.mulT(h);
            w.resize(m - l + 1);
            for(int i = l;i <= m; i++){
                f[i] = (f[i] + w[i - l]) % P;
            ...
            for(int i = l;i <= m;i++){
                for(int j = m + 1;j <= r;j++){
                    if(c[j] == 0)continue;
                    for(int k = 1;i + j * k <= v;k++){
                        f[i] = (f[i] + 1ll * c[j] * f[i + j * k]) % P;
                    }
                }
            }
        }
