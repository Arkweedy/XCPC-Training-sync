...
Z min_25(ll n)
{
    ...
    auto F = [&](auto&&self, ll x, int k)->Z //F(n / x, k)
    {
        ...
        if(x <= m)res += fprime1[x] * 4;
        else res += fprime0[n / x] * 4;

        for(int i = k, p = prime[i]; 1ll * p * p * x <= n;++i, p = prime[i]){
            ll pc = p;
            for(int c = 1; pc * p * x <= n;++c, pc *= p){
                res += Z(c + 1) * (c + 1) * self(self, x * pc, i + 1) + Z(c + 2) * (c + 2);
            }
        }
        return res;
    };
    ...
    auto G = [&](auto&&self, ll x, int k)->Z //F(n / x, k)
    {
        ...
        for(int i = k, p = prime[i]; 1ll * p * p * x <= n;++i, p = prime[i]){
            ll pc = p;
            for(int c = 1; pc * p * x <= n;++c, pc *= p){
                res += Z(c + 1) * self(self, x * pc, i + 1) + Z(c + 2);
            }
        }
        return res;
    };
    ...
    Z res = (F(F, 1, 0) - G(G, 1, 0)) / 2; // \sum d*1  - \sum d
