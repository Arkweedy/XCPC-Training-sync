...
pair<Z,Z> min_25(ll n)
{
    ...
    vector<Z>gprime0(m + 1),gprime1(m + 1);
    for(int i = 1;i <= m;i++){
        gprime0[i] = g0[i] - h0[i];
        gprime1[i] = g1[i] - h1[i];
    }
    ...
    auto F = [&](auto&&self, ll x, int k)->Z //F(n / x, k)
    {
        ...
        for(int i = k, p = prime[i]; 1ll * p * p * x <= n;++i, p = prime[i]){
            ll pc = p;
            res -= self(self, x * pc, i + 1);
        }
        return res;
    };

    //phi(p^k) = p^k - p^(k - 1)
    auto G = [&](auto&&self, ll x, int k)->Z //F(n / x, k)
    {
        Z res = 0;
        if(k > 0)res -= gprime0[prime[k - 1]];
        if(x <= m)res += gprime1[x];
        else res += gprime0[n / x];

        for(int i = k, p = prime[i]; 1ll * p * p * x <= n;++i, p = prime[i]){
            ll pc = p;
            for(int c = 1; pc * p * x <= n;c++, pc *= p){
                res += (Z(pc) - (pc / p)) * self(self, x * pc, i + 1) + Z(pc * p - pc);
            }
        }
        return res;
    };

    return {G(G,1,0) + 1,F(F,1,0) + 1};
}
