...
void solve()
{
    ...
    vector<int>val(len, m + 1);
    vector<int>sqr(len, 0);
    vector<int>ss(len, 0);
    
    for(int i = 0;i < len;i++){
        array<int,4>rec{0,0,h,w};
        for(int j = 0;j < n;j++){
            if(i >> j & 1){
        ...
        if(rec == nar)continue;
        int sq = (rec[2] - rec[0]) * (rec[3] - rec[1]);
        ss[i] = (i == 0 ? 0 : sq);
        for(int s = i;1;s = i & s - 1){
            int dif = popcount(uint(s ^ i));
            cnt[val[s]] += (dif % 2 == 0 ? 1 : -1) * sq;
            sqr[i] += (popcount(uint(s)) % 2 == 1 ? 1 : -1) * ss[s];
            if(s == 0)break;
        }
    }
