...
template<>
constexpr MInt<1004535809> primitiveRoot<1004535809> {702606812};

template<int P>
void ntt(vector<MInt<P>> &a, bool invert) {
    int n = (int)a.size();
    // bit-reverse
    ...
    static MInt<P> root[30], iroot[30];
    if (!inited) {
        const int MAXB = __builtin_ctz(P - 1);       // 对 1004535809 是 21
        MInt<P> g = primitiveRoot<P>;
        for (int k = 0; k < MAXB; k++) {
            // 每层的 zeta = g^{(P-1)/2^{k+1}}
            root[k]  = power(g, (P - 1) >> (k + 1));
            iroot[k] = root[k].inv();
        }
        inited = true;
    }
    for (int len = 1, lvl = 0; 2 * len <= n; len <<= 1, ++lvl) {
        MInt<P> wlen = invert ? iroot[lvl] : root[lvl];
        for (int i = 0; i < n; i += 2 * len) {
            MInt<P> w = 1;
            for (int j = 0; j < len; j++) {
                MInt<P> u = a[i + j];
                MInt<P> v = a[i + j + len] * w;
                a[i + j]       = u + v;
                a[i + j + len] = u - v;
                w *= wlen;
            }
...
void solve()
{
    //cerr<<findPrimitiveRoot<1004535809>()<<endl;
    int n, m, s;
    cin>>n>>m>>s;
    vector<Z>w(m + 1);
    for(int i = 0;i <= m;i++){
    ...
    for(int i = 0;i <= mi;i++){
        g[i] = power(Z(m - i), n - s * i) / comb.fac(m - i) / power(comb.fac(s), i) / comb.fac(n - s * i);
    }

    reverse(f.begin(),f.end());
    auto h = f * g;
    
    poly res(h.begin() + lim, h.end());

    Z ans = 0;
    Z nm = comb.fac(n) * comb.fac(m);
    
    for(int i = 0;i <= m;i++){
        ans += res[i] * w[i] * nm * comb.invfac(i);
    }
    cout<<ans<<endl;
    return;
}
