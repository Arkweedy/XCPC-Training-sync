...
// primitive 4-th root: I^2 = -1 (mod P)
const int I4 = power(G, (P - 1) / 4);

// 已知 W[s + t] = w^t，其中 w 是 2s 次单位根
// 返回 w^{-j}
inline int inv_root_pow(int s, int j) {
    if (j == 0) return 1;
    return P - W[s + (s - j)];
}
    ...
    if (lg & 1) {
        ...
        for (int i = 0; i < n; i += k << 1) {
            for (int j = 0; j < k; j++) {
                ...
                f[i + j] = addmod(x, y);
                f[i + j + k] = 1ll * submod(x, y) * W[k + j] % P;
            }
        }
    }

    // radix-4 DIF
    // k 表示 quarter size，块长为 4k
    for (int k = (lg & 1) ? (n >> 3) : (n >> 2); k; k >>= 2) {
        for (int i = 0; i < n; i += k << 2) {
            for (int j = 0; j < k; j++) {
                ...
                int t1 = submod(a0, a2);
                int t2 = addmod(a1, a3);
                int t3 = 1ll * submod(a1, a3) * I4 % P; // +i

                f[i + j] = addmod(t0, t2);
                f[i + j + k] = 1ll * addmod(t1, t3) * w1 % P;
                f[i + j + (k << 1)] = 1ll * submod(t0, t2) * w2 % P;
                f[i + j + k * 3] = 1ll * submod(t1, t3) * w3 % P;
            }
        }
    }
}
...
void idft(vector<int>& f)
{
    ...
    // radix-4 IDIT
    for (int k = 1, lim = (lg & 1) ? (n >> 3) : (n >> 2); k <= lim; k <<= 2) {
        for (int i = 0; i < n; i += k << 2) {
            for (int j = 0; j < k; j++) {
                int w1 = inv_root_pow(k << 1, j); // (primitive 4k-th root)^(-j)
                int w2 = 1ll * w1 * w1 % P;
                int w3 = 1ll * w2 * w1 % P;

                int a0 = f[i + j];
                int a1 = 1ll * f[i + j + k] * w1 % P;
                int a2 = 1ll * f[i + j + (k << 1)] * w2 % P;
                int a3 = 1ll * f[i + j + k * 3] * w3 % P;

                int t0 = addmod(a0, a2);
                int t1 = submod(a0, a2);
                int t2 = addmod(a1, a3);
                int t3 = 1ll * submod(a1, a3) * (P - I4) % P; // -i

                f[i + j] = addmod(t0, t2);
                f[i + j + k] = addmod(t1, t3);
                f[i + j + (k << 1)] = submod(t0, t2);
                f[i + j + k * 3] = submod(t1, t3);
            }
        }
    }

    // 若 log2(n) 为奇数，最后补一层 radix-2 IDIT
    if (lg & 1) {
        int k = n >> 1;
        for (int i = 0; i < n; i += k << 1) {
            for (int j = 0; j < k; j++) {
                int x = f[i + j];
                int y = 1ll * f[i + j + k] * inv_root_pow(k, j) % P;
