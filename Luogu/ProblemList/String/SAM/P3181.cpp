...
struct SAM
{
    static constexpr int M = 27;//size 
    struct Node{
        int len;
        int link;
        array<int,M>nxt;
...
void solve()
{
    ...
    auto&ot = sam.ot;
    int p = 1;
    vector<int>cnt1(t.size()),cnt2(t.size());
    for(int i = 0;i < s1.length();i++){
        p = t[p].nxt[SAM::num(s1[i])];
        cnt1[p]++;
    }
    p = t[p].nxt[SAM::num('#')];
    for(int i = 0;i < s2.length();i++){
        p = t[p].nxt[SAM::num(s2[i])];
        cnt2[p]++;
    }
    auto dfs = [&](auto&&self, int p)->void
    {
    ...
    for(int i = 2;i < t.size();i++){
        ...
        ans += 1ll * cnt1[i] * cnt2[i] * (t[i].len - t[t[i].link].len);
    }
    cout<<ans<<endl;
    return;
}
