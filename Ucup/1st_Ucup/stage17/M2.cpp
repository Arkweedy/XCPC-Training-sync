...
public:
    ...
    vector<int> eval(vector<int> x)const 
    {
        ...
        if (n <= 64) {
            vector<int> ans(n);
            for (int i = 0; i < n; i++) {
                long long xi = x[i]; // 注意：x[i] 可能等于 P，此时等价于 0（mod P）
                long long v = 0;
                // Horner: v = f(xi)
...
void solve()
{
    ...
    auto build = [&](auto&&self, int l, int r, int p)->void
    {
        ...
        m1[p] = m1[p * 2] * m1[p * 2 + 1];
        return;
    };
    build(build, 0, n - 1, 1);

    vector<int>f(n, 1), g(n, 1);
    auto cdq = [&](auto&&self, int l, int r, int p)->void
    {
        if(l == r){
            return;
        }
            ...
            self(self, m + 1, r, p * 2 + 1);
            vector<int>x1(r - m);
            iota(x1.begin(),x1.end(), m + 1);
            auto v1 = m1[p * 2].eval(x1);
            for(int i = m + 1;i <= r;i++){
                f[i] = 1ll * f[i] * v1[i - m - 1] % P;
            }
            return;
        }   
    };
    ...
    i64 ans = 0;
    for(int i = 0;i < n;i++){
        ans = (ans + 1ll * a[i] * f[i] % P * inv(fac[i]) % P) % P;
    }
