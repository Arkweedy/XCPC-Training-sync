...
void solve()
{
    int p, m;
    cin>>p>>m;
    constexpr Z inv = Z(1e8).inv();
    vector<vector<Z>>pos(p);
    for(int i = 0;i < m;i++){
        int step, x;
        cin>>step>>x;
        Z pp = x * inv;
        pos[step % p].push_back(pp);
    }

    auto norm = [&](Poly<> &poly)->void
    {
        if(poly.size() >= p){
            for(int j = p;j < poly.size();j++){
                poly[j % p] += poly[j];
            }
            poly.resize(p);
        }
    ...
    auto dcntt = [&](auto&&self, int l, int r, int id)->Poly<>
    {
        if(r == -1){
            return Poly<>{1};
        }
        if(l == r){
            return Poly<>{1 - pos[id][l] , pos[id][l]};
        }
        else{
            int m = l + r >> 1;
            auto lpoly = self(self,l,m,id);
            auto rpoly = self(self,m + 1,r,id);
            auto res = lpoly * rpoly;
            norm(res);
            return res;
        }
    };
    ...
    Poly<> ans{1};
    for(int i = 1;i < p;i++){
        auto mpoly = dcntt(dcntt,0,(int)pos[i].size() - 1,i);
        Poly<>ppoly(p);
        for(int j = 0;j < mpoly.size();j++){
            ppoly[i * j % p] = mpoly[j];//1~p-1 are generater of Z_p, so not use +=
        }
