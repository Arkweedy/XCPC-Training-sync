...
//F.cpp Create time : 2026.02.15 14:54
...
void solve()
{
    ...
    vector<i64>dfac(n + 1);
    vector<i64>fac(n + 1);
    vector<i64>invfac(n + 1);
    dfac[0] = fac[0] = invfac[0] = 1;
    for(int i = 1;i <= n;i++){
        fac[i] = 1ll * fac[i - 1] * i % P;
        dfac[i] = 1ll * dfac[i - 1] * ((k - 1 + i) % P) % P;
    }
    invfac[n] = inv(fac[n]);
    for(int i = n - 1;i >= 1;i--){
        invfac[i] = 1ll * invfac[i + 1] * (i + 1) % P;
    ...
    auto binomk = [&](int m)->int
    {
        return 1ll * dfac[m] * invfac[m] % P;
    };
    ...
    auto calc = [&](int p)->void
    {
        ...
        int i = p;
        while(p != -1){
            ans[p] = (ans[p] +  1ll * a[i] * binomk(d)) % P; // += binom(k + d - 1, d) = \frac{k ^ {\upperline{d}}}{d!}
            d++;
            p = fa[p];
        }
    };
