...
## N
链上的石子合并问题
环上问题可以倍长区间进行转化，因此就讨论链上问题。

朴素做法是$O(n^3)$的，区间dp即可

观察到性质: 
当$a \leq b \leq c \leq d$时，有
$$w(a, c) + w(b, d) \leq w(a, d) + w(b, c) \\ w(a, d) \geq w(b, c)$$
(实际上这里可以取等， $w(l,r)$ 是一个 Modular函数) 
(模 (Submodular)、超模 (Supermodular)和模(Modular)函数是组合优化中用到的集合函数概念)

我们有DP递推式：
$$dp_{i, j} = \min_{i \leq k \leq j} (dp_{i,k} + dp_{k + 1, j}) + w(i, j)$$
依据上面我们给出的 $w$ 的两条性质，我们可以得出：
$$dp_{a, c} + dp_{b, d} \leq dp_{a, d} + dp_{b, c} $$
证明大概过程是枚举最佳决策点，进行规约。本质上是Monge
这就是经典的四边形不等式。

从四边形不等式，我们可以推导出决策单调性，然后进行优化。
对于最优决策点$opt[l][r]$，定义为最左/最右均可，但是不能混用。
决策单调性的描述如下：
$$opt[l][r−1]\leq opt[l][r] \leq opt[l+1][r]$$
我们只证一侧的决策单调性, 另一侧是相似的
大概的推导过程是使用反证法，假设$p$是$[l,r]$的最优决策点，$q$是$[l， r - 1]$的最优决策点,假设 $p < q$ 有：
$$\begin{align*}
    dp_{l, p} + dp_{p + 1, r} &\leq dp_{l, q} + dp_{q + 1, r}\\
    dp_{l, q} + dp_{q + 1, r - 1} &\leq dp_{l, p} + dp_{p + 1, r - 1}\\
    \Rightarrow dp_{p + 1, r} + dp_{q + 1, r - 1} & \leq dp_{p + 1, r - 1} + dp_{q + 1, r}\\
\end{align*}$$
又由四边形不等式，有：
$$dp_{p + 1, r} + dp_{q + 1, r - 1} \geq dp_{p + 1, r - 1} + dp_{q + 1, r}$$
因此两个初始不等式两边全部取等，那么对于区间$[l, r]$, $q$ 也是最佳决策点，违反了事先的约定，因此假设不成立，得出决策单调性。

怎么使用决策单调性这个性质来优化dp呢？
这个dp我们是按区间长度递增的顺序来枚举的，求解区间$[l, r]$时，我们枚举决策点的区间范围就可以被限定在 $[opt[l][r - 1], opt[l + 1][r]]$ 这个区间里面。
