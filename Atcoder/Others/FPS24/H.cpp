...
void solve()
{
    int n, m;
    cin>>n>>m;
    //ans =[x^m] 1/(1-x)^n * \sum_{i = 0}^\infty (x/(1-x))^i
    //    =[x^m] 1/(1-x)^n * (1-x)/(1-2x)
    //    =[x^m] 1/(1-x)^{n - 1} * 1/(1-2x) no

    //true ans : [x^m] (1-x)/(1-2x)^(n + 1)
    
    //Z ans = power(Z(2), m - 1) * (comb.binom(m + n, n) + comb.binom(m + n - 1, n - 1));
    Z ans2 = power(Z(2), m) * comb.binom(n + m, m) - power(Z(2), m - 1) * comb.binom(n + m - 1, m - 1);
    //cout<<ans<<endl;
    cout<<ans2<<endl;
    return;
}
