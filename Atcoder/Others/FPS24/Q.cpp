...
void solve()
{
    ...
    auto cdq = [&](auto&&self, vector<Z>&f, int l, int r)->pair<poly,poly>
    {
        if(l == r){
            poly p = {1};
            poly q = {1, -f[l]};
        ...
        int m = l + r >> 1;
        auto [p1,q1] = self(self, f, l, m);
        auto [p2,q2] = self(self, f, m + 1, r);
        poly p = p1 * q2 + p2 * q1;
        poly q = q1 * q2;
        return make_pair(p, q);
    };

    auto [pa, qa] = cdq(cdq, a, 0, n - 1);
    auto [pb, qb] = cdq(cdq, b, 0, m - 1);
    
    poly f = pa * qa.inv(k + 1);
    poly g = pb * qb.inv(k + 1);
    for(int i = 0;i <= k;i++){
        f[i] *= comb.invfac(i);
        g[i] *= comb.invfac(i);
    }
