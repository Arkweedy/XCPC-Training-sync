...
void solve()
{
    ...
    poly f(n + 1);
    for(int i = 0;i <= n;i++){
        f[i] = power(Z(2), 1ll * i * (i - 1)) * comb.invfac(i);
    }
    auto arrow = [&](poly& f, int sign)->void
    {
        if(sign == 1){
            for(int i = 0;i <= n;i++){
                f[i] *= power(comb.inv(2), 1ll * i * (i - 1) / 2);
            }
        }
        else if(sign == -1){
            for(int i = 0;i <= n;i++){
                f[i] *= power(Z(2), 1ll *  i * (i - 1) / 2);
            }
        }
        else{
            assert(0);
    ...
    int m = g.cnt;
    auto& bel = g.bel;
    vector<vector<int>>ng(m,vector<int>(m));
    ...
    for(int i = 0;i < n;i++){
        for(int j = 0;j < n;j++){
            if(a[i][j] == 1 && bel[i] != bel[j]){
                ng[bel[i]][bel[j]] = 1;
            }
            if(a[i][j] == 0 && bel[i] == bel[j]){
                flg = 1;
            }
        }
    }
    ...
    for(int i = 0;i < n;i++){
        for(int j = 0;j < n;j++){
            if(bel[i] != bel[j] && a[i][j] == 1){
                if(adj[bel[i]][bel[j]] == 0){
                    flg = 1;
                }
            }
            else if(bel[i] != bel[j] && a[i][j] == 0){
                if(adj[bel[i]][bel[j]] != 0){
                    flg = 1;
                }
            }
